...
-- Create the customer_nodes table
...
-- 3. Customers per region
SELECT r.region_name, COUNT(DISTINCT customer_id) AS customer_count
FROM customer_nodes cn
JOIN regions r ON cn.region_id = r.region_id
GROUP BY r.region_name;

-- 4. Average reallocation duration (end_date - start_date)
SELECT ROUND(AVG(DATEDIFF(end_date, start_date)), 2) AS avg_reallocation_days
FROM customer_nodes
WHERE end_date IS NOT NULL;

-- 5. Median, 80th, 95th percentiles (approximation using GROUP_CONCAT)
SELECT region_name,
       SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(duration ORDER BY duration), ',', ROUND(COUNT(*) * 0.5)), ',', -1) AS median_days,
       SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(duration ORDER BY duration), ',', ROUND(COUNT(*) * 0.8)), ',', -1) AS p80_days,
       SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(duration ORDER BY duration), ',', ROUND(COUNT(*) * 0.95)), ',', -1) AS p95_days
FROM (
  SELECT r.region_name, DATEDIFF(end_date, start_date) AS duration
  FROM customer_nodes cn
  JOIN regions r ON cn.region_id = r.region_id
  WHERE end_date IS NOT NULL
) sub
...
SELECT txn_type, COUNT(*) AS txn_count, SUM(txn_amount) AS total_amount
FROM customer_transactions
GROUP BY txn_type;

-- 2. Avg deposit count and amount per customer
SELECT 
...
-- 3. Monthly customers with >1 deposit and ≥1 withdrawal or purchase
SELECT txn_month, COUNT(DISTINCT customer_id) AS qualified_customers
FROM (
  SELECT customer_id,
         DATE_FORMAT(txn_date, '%Y-%m') AS txn_month,
         SUM(txn_type = 'deposit') AS deposit_count,
         SUM(txn_type IN ('withdrawal', 'purchase')) AS other_txn_count
  FROM customer_transactions
...
-- 4. Closing balance per customer at end of each month
SELECT customer_id, txn_month, SUM(txn_amount) AS closing_balance
FROM (
...
-- 5. Percentage of customers with >5% increase in closing balance (month-to-month)
...
growth_calc AS (
  SELECT m1.customer_id, m1.txn_month AS month1, m1.balance AS bal1,
         m2.txn_month AS month2, m2.balance AS bal2,
         ROUND(((m2.balance - m1.balance) / m1.balance) * 100, 2) AS pct_growth
  FROM monthly_balances m1
  JOIN monthly_balances m2 
    ON m1.customer_id = m2.customer_id AND m2.txn_month > m1.txn_month
)
SELECT 
  ROUND(COUNT(DISTINCT customer_id) * 100 / 
        (SELECT COUNT(DISTINCT customer_id) FROM customer_transactions), 2) AS percent_increased
FROM growth_calc
WHERE pct_growth > 5;

-- Section C
-- 1. Running balance per transaction
SELECT customer_id, txn_date, txn_type, txn_amount,
       SUM(txn_amount) OVER (PARTITION BY customer_id ORDER BY txn_date) AS running_balance
FROM customer_transactions;

-- 2. End-of-month balances
SELECT customer_id, DATE_FORMAT(txn_date, '%Y-%m') AS txn_month,
       SUM(txn_amount) AS month_end_balance
FROM customer_transactions
GROUP BY customer_id, txn_month;

-- 3. Min, Avg, Max running balance per customer
SELECT customer_id,
...
-- 4. Monthly provisioned data for:
-- Option 1: Based on month-end balance
SELECT txn_month, ROUND(SUM(month_end_balance), 2) AS total_data_required
FROM (
  SELECT customer_id, DATE_FORMAT(txn_date, '%Y-%m') AS txn_month,
         SUM(txn_amount) AS month_end_balance
  FROM customer_transactions
...
-- Option 2: Based on 30-day avg balance (approximate by average txn amount over last 30 days)
-- This is best implemented in a procedural script; here's an approximation:
SELECT txn_day, ROUND(SUM(avg_30d_balance), 2) AS data_required
FROM (
  SELECT customer_id, txn_date AS txn_day,
         AVG(txn_amount) OVER (PARTITION BY customer_id ORDER BY txn_date ROWS BETWEEN 29 PRECEDING AND CURRENT ROW) AS avg_30d_balance
  FROM customer_transactions
) sub
GROUP BY txn_day;

-- Option 3: Real-time = total running balance per day
SELECT txn_date, ROUND(SUM(running_balance), 2) AS total_realtime_balance
FROM (
  SELECT customer_id, txn_date,
         SUM(txn_amount) OVER (PARTITION BY customer_id ORDER BY txn_date) AS running_balance
  FROM customer_transactions
...
-- Section D
SELECT txn_date,
       ROUND(SUM(balance) * 0.000164, 2) AS interest_earned
FROM (
  SELECT customer_id, txn_date,
...
WITH RECURSIVE daily_interest AS (
  SELECT 
    ...
    txn_date AS current_date
  FROM customer_transactions
  WHERE (customer_id, txn_date) IN (
    SELECT customer_id, MIN(txn_date)
    FROM customer_transactions
    GROUP BY customer_id
  )
  ...
  SELECT 
    ...
    t.txn_date,
    t.txn_amount,
    ROUND(di.balance * (1 + 0.000164) + t.txn_amount, 2) AS balance,
    t.txn_date AS current_date
  FROM customer_transactions t
  JOIN daily_interest di 
    ON t.customer_id = di.customer_id
   AND t.txn_date = DATE_ADD(di.current_date, INTERVAL 1 DAY)
)
SELECT 
  current_date, 
  SUM(balance) AS total_data_required
FROM daily_interest
